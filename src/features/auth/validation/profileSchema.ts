...
function formNumber(build: (schema: z.ZodNumber) => z.ZodNumber) {
...
const profileFieldsSchema = z.object({
  ...
  carbsGoal: formNumber((n) =>
    n.min(50, 'Minimum carbs goal is 50 g').max(800, 'Maximum carbs goal is 800 g'),
  ),
  fatGoal: formNumber((n) =>
    n.min(20, 'Minimum fat goal is 20 g').max(300, 'Maximum fat goal is 300 g'),
  ),
  fiberGoal: formNumber((n) =>
    n.min(15, 'Minimum fiber goal is 15 g').max(80, 'Maximum fiber goal is 80 g'),
  ),
});
